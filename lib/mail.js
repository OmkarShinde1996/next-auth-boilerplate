...
export async function sendPasswordResetEmail(email, token) {
    ...
    const resetPassEmail = render(<WrappletResetPasswordEmail resetPasswordLink={`${process.env.NEXT_PUBLIC_NEXTAUTH_URL}resetPassword/${token}`} />);
...
export async function sendPasswordResetSuccessEmail(email) {
    console.log({email});
    const resetPassSuccessEmail = render(<WrappletResetPasswordSuccessEmail resetPassUrl={`${process.env.NEXT_PUBLIC_NEXTAUTH_URL}resetPassword`} supportUrl={'#'} updatedDate={new Date()}/>);
    const info = await transporter.sendMail({
        from: from,
        to: email,
